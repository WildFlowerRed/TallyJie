...
TallyJie 是一个 **日记 + 记账 + ToDo + 周计划 + 情绪记录** 一体化的个人生活管理应用。

与传统记账软件不同，TallyJie 不追求冰冷的数字报表，而是强调 **今天发生了什么、心情怎么样、完成了哪些事情**——最终形成一本有温度的**生活账本**。
...
项目使用 sqflite（SQLite）作为本地存储方案。数据库文件存储在应用文档目录下的 `tallyjie.db`。
